...
def run_benchmark_variant(
    ...
    exitcode_path = Path(variant_log_dir, "exitcode.log")
    ...
    while need_to_run:
        if args.reuse_logs:
            stdout, stderr, exitcode, monitor_log = load_from_logs(args.reuse_logs)
...
def benchmarks_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Path to the logs to reuse for metrics extraction/calculations. "
            "This must be a path to existing logs created by this benchmarking "
            "tool."
        ),
    )
    parser.add_argument(
